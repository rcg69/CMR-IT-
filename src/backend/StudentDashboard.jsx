...
const StudentDashboard = () => {
    ...
    const fetchAttendance = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `http://localhost:5000/api/student/attendance-summary?email=${encodeURIComponent(
            student.email
          )}`
        );
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `http://localhost:5000/api/student/attendance-records?email=${encodeURIComponent(
            student.email
          )}`
        );
        const data = await res.json().catch(() => ({}));
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        const res = await fetch(
          `http://localhost:5000/api/student/profile?email=${encodeURIComponent(
            student.email
          )}`
        );
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      const fullUrl = `http://localhost:5000${data.profileImageUrl}`;
  ...
  const dayStatusMap = useMemo(() => {
    ...
    records.forEach((r) => {
      const d = r.date;
      if (!map[d]) map[d] = { present: 0, absent: 0, slots: {} };
  ...
  const displayImage = imagePreviewUrl || storedImage;
  const avatarInitial = (student.name || student.email || 'S')[0].toUpperCase();
              ...
              >
                  ...
                  <div>
                    ...
                    >
                      {student.mentorTeacherEmail
                        ? `Mentor: ${student.mentorTeacherEmail}`
                        : 'Mentor not assigned'}
                    </p>
                  </div>
                </div>
                ...
                <div className="student-file-row">
                  <label className="student-file-label">
                    ...
                    <span className="student-file-text">
            ...
            <div className="student-dashboard-grid">
              ...
              <div className="student-info-card">
                ...
                <div className="student-info-value">
                  {attLoading
                    ? 'Loading...'
                    : `${attendance.percentage}% (${attendance.present}/${attendance.totalClasses})`}
                </div>
                {attError && (
                  <div className="student-profile-warning">{attError}</div>
                )}
              </div>
            </div>
          </div>

          {/* Attendance calendar – unchanged */}
          {/* (this is exactly your calendar block from paste.txt) */}
          {/* ---------------------------------------------------- */}
