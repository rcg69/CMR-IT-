...
const StudentDashboard = () => {
  ...
  const [attendance, setAttendance] = useState({ totalClasses: 0, present: 0, absent: 0, percentage: 0 });
  ...
  const [profile, setProfile] = useState({
    phone: '', branch: '', year: '', section: '', address: '', interests: '',
    guardianName: '', guardianPhone: '', bloodGroup: '', extraInfo: '', profileImageUrl: ''
  });
  ...
  useEffect(() => {
    const fetchAttendance = async () => {
      ...
      try {
        setAttLoading(true);
        setAttError('');
        const res = await fetch(`https://cmr-it.onrender.com/api/student/attendance-summary?email=${encodeURIComponent(student.email)}`);
  ...
  useEffect(() => {
    const fetchRecords = async () => {
      ...
      try {
        setRecLoading(true);
        setRecError('');
        const res = await fetch(`https://cmr-it.onrender.com/api/student/attendance-records?email=${encodeURIComponent(student.email)}`);
        const data = await res.json();
  ...
  useEffect(() => {
    const fetchProfile = async () => {
      ...
      try {
        ...
        setProfileError('');
        setProfileMessage('');
        const res = await fetch(`https://cmr-it.onrender.com/api/student/profile?email=${encodeURIComponent(student.email)}`);
        if (res.status === 404) {
          setProfileLoading(false);
          return;
        }
  ...
  const handleImageUpload = async () => {
    ...
    try {
      ...
      const res = await fetch('https://cmr-it.onrender.com/api/student/profile-image', {
        method: 'POST',
        body: formData
      });
      const data = await res.json();
      if (!res.ok) throw new Error(data.message || 'Failed to upload image');
      const fullUrl = `https://cmr-it.onrender.com${data.profileImageUrl}`;
  ...
  const handleProfileSubmit = async (e) => {
    ...
    try {
      ...
      const res = await fetch('https://cmr-it.onrender.com/api/student/profile', {
        method: 'POST',
  ...
  const displayImage = imagePreviewUrl || (storedImage && storedImage.startsWith('http') ? storedImage : `https://cmr-it.onrender.com${storedImage}`);
  const avatarInitial = (student.name || student.email[0]).toUpperCase();
    ...
    <div className="student-dashboard-page">
      <div className="student-dashboard-container">
        <div className="student-dashboard-card">
          ...
          {/* Profile Summary Row */}
          <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0, 2.2fr) minmax(0, 1.8fr)', gap: '1.5rem', marginTop: '0.5rem' }}>
            
            {/* Profile section - hidden until Show Info clicked */}
            {showProfileInfo && (
              <div style={{ borderRadius: 'var(--radius-lg)', border: '1px solid rgba(255,255,255,0.06)', padding: '1rem 1.25rem', background: 'rgba(12, 18, 40, 0.8)' }}>
                {/* Avatar + basic info */}
                ...
                {/* File input + upload button styled via CSS */}
                ...
                <form onSubmit={handleProfileSubmit} className="student-profile-form">
                  <div className="student-profile-grid2">
                    <div>
                      ...
                      <input type="text" name="phone" value={profile.phone} onChange={handleProfileChange} className="student-profile-input" />
                    </div>
                    <div>
                      <label className="student-profile-label">Branch</label>
                      <input type="text" name="branch" value={profile.branch} onChange={handleProfileChange} className="student-profile-input" />
                    </div>
                    <div>
                      <label className="student-profile-label">Year</label>
                      <input type="text" name="year" value={profile.year} onChange={handleProfileChange} className="student-profile-input" />
                    </div>
                    <div>
                      <label className="student-profile-label">Section</label>
                      <input type="text" name="section" value={profile.section} onChange={handleProfileChange} className="student-profile-input" />
                    </div>
                  </div>
                  <div>
                    <label className="student-profile-label">Interests (comma separated)</label>
                    <input type="text" name="interests" value={profile.interests} onChange={handleProfileChange} className="student-profile-input" />
                  </div>
                  <div className="student-profile-grid2">
                    <div>
                      <label className="student-profile-label">Guardian Name</label>
                      <input type="text" name="guardianName" value={profile.guardianName} onChange={handleProfileChange} className="student-profile-input" />
                    </div>
                    <div>
                      <label className="student-profile-label">Guardian Phone</label>
                      <input type="text" name="guardianPhone" value={profile.guardianPhone} onChange={handleProfileChange} className="student-profile-input" />
                    </div>
                  </div>
                  <div className="student-profile-grid2">
                    <div>
                      <label className="student-profile-label">Blood Group</label>
                      <input type="text" name="bloodGroup" value={profile.bloodGroup} onChange={handleProfileChange} className="student-profile-input" />
                    </div>
                    <div>
                      <label className="student-profile-label">Address</label>
                      <textarea name="address" value={profile.address} onChange={handleProfileChange} rows={2} className="student-profile-textarea" />
                    </div>
                  </div>
                  <div>
                    <label className="student-profile-label">Extra Info</label>
                    <textarea name="extraInfo" value={profile.extraInfo} onChange={handleProfileChange} rows={2} className="student-profile-textarea" />
                  </div>
                  
                  {profileError && <div className="student-profile-error">{profileError}</div>}
                  {profileMessage && <div className="student-profile-success">{profileMessage}</div>}
                  
                  <button type="submit" className="student-dashboard-primary-btn" disabled={profileSaving || profileLoading}>
            ...
            <div className="student-dashboard-grid">
              ...
              <div className="student-info-card">
                ...
                {attError && <div className="student-profile-warning">{attError}</div>}
              </div>
            </div>
          </div>

          {/* Attendance calendar - unchanged (this is exactly your calendar block from paste.txt) */}
          <div style={{ marginTop: '2rem' }}>
            ...
            {recError && (
              <div style={{
                ...
                borderRadius: 'var(--radius-md)', color: '#ef4444', fontSize: '0.85rem'
              }}>
                {recError}
              </div>
            )}
            ...
            <div style={{
              ...
              gap: '0.35rem', fontSize: '0.8rem', marginBottom: '0.5rem',
              ...
              {calendarDays.map((info, idx) => {
                ...
                return (
                  <button
                    ...
                    style={{
                      minHeight: '40px', borderRadius: '10px', padding: '0.25rem 0.35rem',
                      background: color,
                      border: isSelected ? '2px solid rgba(129, 178, 241, 0.9)' : '1px solid rgba(255, 255, 255, 0.06)',
                  ...
                  >
                    ...
                    <div style={{ fontSize: '0.65rem', color: 'var(--text-muted)', textAlign: 'left' }}>
                      {Object.keys(info.status.slots).length === 0 ? 'Slots: 0' : `Slots: ${Object.keys(info.status.slots).length}`}
                    </div>
                  </button>
                );
              })}
            ...
            <div style={{ marginTop: '1.5rem' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '0.4rem', fontSize: '0.8rem' }}>
                {Array.from({ length: 8 }).map((_, idx) => {
                  ...
                  const bg = status === 'present' ? 'rgba(34, 197, 94, 0.2)' :
                            status === 'absent' ? 'rgba(239, 68, 68, 0.2)' :
          ...
          <div style={{ marginTop: '1.75rem', display: 'flex', gap: '0.75rem', flexWrap: 'wrap' }}>
            <button
              className="student-dashboard-primary-btn"
              onClick={() => alert(`Attendance: ${attendance.percentage}% (${attendance.present}/${attendance.totalClasses})`)}
            >
